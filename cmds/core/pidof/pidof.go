...
func run(stdout io.Writer, procPath string, single, quite bool, args []string) error {
...
func main() {
	...
	quite := flag.Bool("q", false, "do not display matched PIDs to standard out")
	flag.Parse()
	if err := run(os.Stdout, procPath, *single, *quite, flag.Args()); err != nil {
