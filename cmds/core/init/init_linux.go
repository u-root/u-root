...
func osInitGo() *initCmds {
	...
	multitty := libinit.WithMultiTTYOutput(cmdline.Consoles())
	...
	return &initCmds{
		cmds: []*exec.Cmd{
			...
			libinit.Command("/inito", libinit.WithCloneFlags(syscall.CLONE_NEWPID), ctty, multitty),

			libinit.Command("/bbin/uinit", ctty, multitty, uinitArgs),
			libinit.Command("/bin/uinit", ctty, multitty, uinitArgs),
			libinit.Command("/buildbin/uinit", ctty, multitty, uinitArgs),

			libinit.Command("/bin/defaultsh", ctty, multitty),
			libinit.Command("/bin/sh", ctty, multitty),
		},
	}
}
