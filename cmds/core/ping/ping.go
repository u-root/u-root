...
//     -a: audible rings a terminal bell when a packet is received
//     -h: help
package main
...
var (
	...
	audible    = flag.Bool("a", false, "audible rings a terminal bell when a packet is received")
)
...
func usage() {
	fmt.Fprintf(os.Stdout, "ping [-V] [-a] [-6] [-c count] [-i interval] [-s packetsize] [-w deadline] destination\n")
	os.Exit(0)
}
