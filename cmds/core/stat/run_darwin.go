...
const defFormat = "%d %d %s %d %d %d %d %d %q %q %q %q %d %d %d %s\n"
...
func run(stdout io.Writer, stderr io.Writer, files ...string) int {
	...
	for _, file := range files {
		...
		fmt.Fprintf(stdout, defFormat,
			...
			stat.Rdev,
			stat.Size,
			fromTimespec(stat.Atimespec),
			fromTimespec(stat.Mtimespec),
			fromTimespec(stat.Ctimespec),
			fromTimespec(stat.Birthtimespec),
			stat.Blksize,
			stat.Blocks,
			stat.Flags,
			fi.Name(),
