...
type ProcessTable struct {
	...
	header  []string  // each column to print
...
func (pT ProcessTable) PrintHeader() {
	for index, field := range pT.header {
...
func (pT *ProcessTable) PrepareString(cW ColumnsWidth) {
	var (
		fstring  []string
		formated string
		width    int
	)
	for _, f := range pT.header {
		switch f {
		case "PID":
			formated, width = "%%-%dv ", cW.PID
		case "TTY":
			formated, width = "%%-%dv\t ", cW.TTY
		case "STAT":
			formated, width = "%%-%dv\t", cW.STAT
		case "TIME":
			formated, width = "%%-%dv ", cW.TIME
		case "CMD":
			formated, width = "%%-%dv ", cW.CMD
		}
		fstring = append(fstring, fmt.Sprintf(formated, width))
	}
...
// used to make a pretty output like GNU ps
...
func ps(pT ProcessTable) error {
	...
	if flags.x {
		pT.header = []string{"PID", "TTY", "STAT", "TIME", "CMD"}
		pT.fields = []string{"Pid", "Ctty", "State", "Time", "Tcomm"}
	} else {
		pT.header = []string{"PID", "TTY", "TIME", "CMD"}
