...
func createDir(src, dst string) error {
	...
	if os.IsNotExist(err) {
		...
		return Errorf("can't overwrite non-dir '%v' with dir '%v'\n", dst, src)
	}
...
func copyDir(src, dst string) error {
	...
	if err != nil {
		return Errorf("can't list files from '%v': '%v'\n", src, err)
	}
...
func main() {
	...
	if err == nil {
		...
		cwd, err := os.Getwd()
		if err != nil {
			log.Printf("can't get the current directory: %v\n", err)
		}
		createDir(cwd, lf)
	} else {
		log.Printf("can't open the %v file: %v\n", err)
	}
