...
func TestCpSymlink(t *testing.T) {
	...
	t.Logf("Enter directory: '%v'", tempDir)
	// Enter to temp directory to don't have problems
	// with copy links and relative paths
	os.Chdir(tempDir)
	defer os.Chdir("..")
	defer t.Logf("Exiting directory: '%v'", tempDir)

	t.Logf("Create link '%v' -> '%v'", srcFname, linkName)
	...
	t.Logf("Copy from: '%v'", linkName)
	t.Logf("To: '%v'", dstFname)
	if err := copyFile(linkName, dstFname, false); err != nil {
		t.Fatalf("copyFile '%v' -> '%v' failed: %v", linkName, dstFname, err)
	}
	...
	if !isEqualFile(s, d, t) {
		t.Fatalf("checksum are different; copies failed %v -> %v", linkName, dstFname)
	}
}
