...
func TestFS9(t *testing.T) {
	...
	for _, p := range []string{"fd", "dev"} {
		...
		if err := os.MkdirAll(dd, 0777); err != nil {
	...
	for _, tt := range []struct {
		...
		{name: "bad sd0ctl", make: func() error { return os.WriteFile(filepath.Join(d, "fd/sd0ctl"), nil, 0666) }, err: os.ErrNotExist, f: func() error {
			_, err := consctl(d, 0)
			return err
		},
		},
		{name: "good sd0ctl", make: func() error { return os.WriteFile(filepath.Join(d, "fd/0ctl"), []byte("a b "+devcons), 0666) }, err: nil, f: func() error {
			n, err := consctl(d, 0)
			if err != nil {
				return err
			}
		...
		{name: "good /dev/consctl", make: func() error { return os.WriteFile(ctl, nil, 0666) }, err: nil, f: func() error {
			_, err := consctlFile(d, 0)
			return err
		},
		},
		...
		{name: "bad wctl", make: func() error { return os.WriteFile(wctl, []byte("12 33 11 "), 0666) }, err: io.ErrUnexpectedEOF, f: func() error {
			_, _, err := readWinSize(wctl)
			return err
		},
		},
		{name: "good wctl", make: func() error { return os.WriteFile(wctl, window, 0666) }, err: nil, f: func() error {
