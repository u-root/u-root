...
// InterfaceExists returns a CheckRunner that verifies if an interface is present on
// the system
func InterfaceExists(ifname string) error {
...
// megabit/s.
func LinkSpeed(ifname string, minSpeed uint32) error {
...
func LinkAutoneg(ifname string, expected bool) error {
	ec := ethtool.EthtoolCmd{}
...
// InterfaceHasLinkLocalAddress returns a CheckRunner that verifies if an interface
// has a configured link-local address.
func InterfaceHasLinkLocalAddress(ifname string) error {
...
// InterfaceHasGlobalAddresses returns a CheckRunner that verifies if an interface has
// at least one global address.
func InterfaceHasGlobalAddresses(ifname string) error {
	addrs, err := addresses(ifname)
	if err != nil {
		return err
	}
...
// InterfaceRemediate returns a RemediationRunner that tries to fix a missing
// interface issue.
func InterfaceRemediate(ifname string) error {
...
func InterfaceCanDoDHCPv6(ifname string) error {
