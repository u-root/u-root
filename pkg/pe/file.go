...
var (
	...
	sizeofDataDirectory    = uint16(binary.Size(DataDirectory{}))
)
...
func NewFile(r io.ReaderAt) (*File, error) {
	...
	sr.Seek(-2, seekCurrent)
	var oh32 OptionalHeader32
	var oh64 OptionalHeader64
	var numOfDataDirs uint16
	switch magic {
	case 0x10b: // PE32
		...
		numOfDataDirs = (f.FileHeader.SizeOfOptionalHeader - sizeofOptionalHeader32) / sizeofDataDirectory
	...
	case 0x20b: // PE32+
		...
		numOfDataDirs = (f.FileHeader.SizeOfOptionalHeader - sizeofOptionalHeader64) / sizeofDataDirectory

	default:
		return nil, fmt.Errorf("pe header has unknown magic %#x", magic)
	}
	f.DataDirectories = make([]DataDirectory, numOfDataDirs)
	for i := uint16(0); i < numOfDataDirs; i++ {
		if err := binary.Read(sr, binary.LittleEndian, &f.DataDirectories[i]); err != nil {
			return nil, err
		}
	}
