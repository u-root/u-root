...
// TestConvenienceFunctions tests the Read/Write/WriteThenRead functions which
// wrap the Transfer function.
func TestConvenienceFunctions(t *testing.T) {
	...
	t.Run("Read", func(t *testing.T) {
		...
		readBuf := []uint8{0, 0}
		if gotErr := s.Read(readBuf); gotErr != nil {
			t.Errorf("Got Read err %v; want nil", gotErr)
		}
		if !reflect.DeepEqual(m.transfers, wantTransfers) {
			t.Errorf("Got Read transfers %#v; want %#v", m.transfers, wantTransfers)
		}
	})
	...
	t.Run("Write", func(t *testing.T) {
		...
		writeBuf := []uint8{2, 3}
		if gotErr := s.Write(writeBuf); gotErr != nil {
			t.Errorf("Got Write err %v; want nil", gotErr)
		}
		if !reflect.DeepEqual(m.transfers, wantTransfers) {
			t.Errorf("Got Write transfers %#v; want %#v", m.transfers, wantTransfers)
		}
	})
	...
	t.Run("WriteThenRead", func(t *testing.T) {
		...
		writeBuf := []uint8{2, 3, 4}
		readBuf := []uint8{0, 0}
		if gotErr := s.WriteThenRead(writeBuf, readBuf); gotErr != nil {
			t.Errorf("Got WriteThenRead err %v; want nil", gotErr)
		}
		if !reflect.DeepEqual(m.transfers, wantTransfers) {
			t.Errorf("Got WriteThenRead transfers %#v; want %#v", m.transfers, wantTransfers)
		}
	})
}
