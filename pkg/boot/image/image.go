...
// Package image contains a parser for Arm64 Linux format. It
...
type Image struct {
	Header Arm64Header
	// TODO(10000TB): specify other data segments.
}
...
func ParseFromBytes(data []byte) (*Image, error) {
	...
	if img.Header.Magic != Magic {
		return img, fmt.Errorf("invalid magic: %#08x, expected: %08x", img.Header.Magic, Magic)
	}
